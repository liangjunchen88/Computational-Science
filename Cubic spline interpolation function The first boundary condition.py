...
#输入第一种边界条件、XY的取值
print('第一种边界条件')
str0 = input("请分别输入M[0], M[n]的取值，用空格隔开：")  
list0 = str0.split(" ") 
...
#列出三弯矩方程组的系数矩阵
P_lenth = len(P)
A = [[0 for i in range(P_lenth)] for i in range(P_lenth)]
A[0][0] = 2
A[0][1] = Q[0]
A[P_lenth-1][P_lenth-1] = 2
A[P_lenth-1][P_lenth-2] = P[P_lenth-1]
i = 1
while i <= P_lenth - 2:
    A[i][i] = 2
    A[i][i-1] = P[i]
...
B = [0]*(P_lenth)
B[0] = d[0] - P[0]*m[0]
B[P_lenth - 1] = d[P_lenth - 1] - Q[P_lenth - 2]*m[1]
i = 1
while i <= len(B)-2:
    B[i] = d [i]
    i += 1
...
while i < len(M) :
    print("S(x) = ",(M[i]-M[i-1])/(6*H[i-1]),"x**3 + ",
          ...
          ( M[i]*(X[i-1]**2)-M[i-1]*(X[i]**2))/(2*H[i-1])
          + (Y[i] - Y[i-1])/H[i-1] + H[i-1]*(M[i-1]-M[i])/6,
          "x + ",(M[i-1]*(X[i]**3)-M[i]*(X[i-1]**3))/(6*H[i-1])
          + ( Y[i-1]*X[i] -Y[i]*X[i-1] )/H[i-1] 
          +  H[i-1]*(M[i]*X[i-1] - M[i-1]*X[i])/6 
          , "     ",X[i-1],"<=",X[i] ) 
    i += 1
